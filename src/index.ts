...
const BRICK_WIDTH : number = 100
const BRICK_HEIGHT : number = 20

const SCREEN_WIDTH : number = 780 
const SCREEN_HEIGHT : number = 500

const BALL_DIAMETER :number = 25
...
//display the ball
 ...
 checkForCollisions() {
    ...
    for (let i=0; i< bricks.length; i++){
        if (
            (ballPosition[0] > bricks[i].bottomLeft[0] && ballPosition[0] < bricks[i].bottomRight[0]) &&
            ...
            score++
            SCORE_DISPLAY!.innerHTML = "Score: " + (score as unknown as string)
    ...
    if (
        (ballPosition[0]> paddlePosition[0] && ballPosition[0]< paddlePosition[0] + BRICK_WIDTH) &&
    ...
    if (
        ballPosition[1] >= (SCREEN_HEIGHT - BALL_DIAMETER)
        ) {
            this.changeDirection()
    }

    if (
        ballPosition[0] >= (SCREEN_WIDTH - BALL_DIAMETER) ||
        ballPosition[0] <=0
        ) {
            this.changeDirection()
    }
...
function toDoubleDigits(x : number){
    if(x < 10)
        return "0" + (x as unknown as string)
